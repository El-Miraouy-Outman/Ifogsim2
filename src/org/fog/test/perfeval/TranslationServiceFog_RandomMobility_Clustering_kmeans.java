...
public class TranslationServiceFog_RandomMobility_Clustering_kmeans {
    // ajouter par el miraouy
    static int numberOfGatway=0;
    static int numberOfProxy=0;
    ...
    public static int numberOfMobileUser = Variable.numberUser;
    ...
    private static void createFogDevices(int userId, String appId) throws NumberFormatException, IOException {
        ...
        if (locator.getLevelWiseResources(locator.getLevelID("Cloud")).size() == 1) {
           ...
           // System.out.println("*-*-*-*--*-**-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-* cloud"+ locator.getLevelWiseResources(locator.getLevelID("Cloud")).get(0));
            ...
            //System.out.println("________ boucle    proxy");

            for (int i = 0; i < locator.getLevelWiseResources(locator.getLevelID("Proxy")).size(); i++) {
                // creates the fog device Proxy Server (level=1)
                FogDevice proxy = createFogDevice("proxy-server_" + i,Variable.mips * 2800, Variable.ram * 4000, 10000,  10000,  0.0, 107.339, 83.4333);
            ...
            for (int i = 0; i < locator.getLevelWiseResources(locator.getLevelID("Gateway")).size(); i++) {
                ...
                //System.out.println("*-*-*-*--*-**-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-* gatewy id "  + gateway.getId());
