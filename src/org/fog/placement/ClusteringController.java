...
import org.printInExcel.CreationFichierExcel;
...
public class ClusteringController extends SimEntity {
    ...
    public void processEvent(SimEvent ev) {
        ...
        switch (ev.getTag()) {
            ...
            case FogEvents.STOP_SIMULATION:
                ...
                CreationFichierExcel.writeInExcel(
                        executionTime,
                        ACTION_COMMAND,
                        RAW_DATA,
                        PROCESSED_DATA,
    ...
    private void printMigrationDelayDetails() {
        this.migrationTime= MigrationDelayMonitor.getMigrationDelay();
        // TODO Auto-generated method stub
        System.out.println("Total time required for module migration for clustering = " + MigrationDelayMonitor.getMigrationDelay());
    }
    ...
    private void processMobility(SimEvent ev) {
        ...
        if (prevParent.getId() != newParent.getId()) {
            ...
            // System.out.println("Child* " + fogDevice.getName() + "\t----->\t new Parent " + newParent.getName() +"    previous :: "+prevParent.getName());
            ...
            // System.out.println("--------------- Application :: "+ fogDevice.getActiveApplications());
    ...
    private void printNetworkUsageDetails() {
        this.network_usage=NetworkUsageMonitor.getNetworkUsage() / Config.MAX_SIMULATION_TIME;
    ...
    private void printCostDetails() {
        this.Cost_Of_Execution=getCloud().getTotalCost();
    ...
    */
    private void printTimeDetails() {
        ...
        this.executionTime = Calendar.getInstance().getTimeInMillis()
                - TimeKeeper.getInstance().getSimulationStartTime();
        System.out.println("=========================================");
        ...
        for (String tupleType : TimeKeeper.getInstance().getTupleTypeToAverageCpuTime().keySet()) {
            ...
            if(i==0){
                this.ACTION_COMMAND=TimeKeeper.getInstance().getTupleTypeToAverageCpuTime().get(tupleType);
            }
            if(i==1){
                this.RAW_DATA=TimeKeeper.getInstance().getTupleTypeToAverageCpuTime().get(tupleType);
            }
            if(i==2){
                this.PROCESSED_DATA=TimeKeeper.getInstance().getTupleTypeToAverageCpuTime().get(tupleType);
            }
            if(i==3){
                this.M_SENSOR=TimeKeeper.getInstance().getTupleTypeToAverageCpuTime().get(tupleType);
            }
            i++;
        }
