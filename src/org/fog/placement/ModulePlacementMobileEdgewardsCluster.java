				...
				if(upsteamDeviceId > 0){
					if(upsteamDeviceId==deviceId){
						...
						if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){
							...
							System.out.printf("??????????__si (totalCpuLoad %f /  getCurrentCpuLoad().get(deviceId )= %f :  somme = %f > device.getHost().getTotalMips()=%d !! \n", totalCpuLoad,getCurrentCpuLoad().get(deviceId),(totalCpuLoad + getCurrentCpuLoad().get(deviceId)), device.getHost().getTotalMips());
							System.out.println(" ???????????===>Placement of operator "+moduleName+ " on device "+device.getName() + " Echec.");
